...
for i in {00..23}
do
var1[10#$i]=$(predict -t ~/wxsat/stations.txt -p "${bird}" $(date -d "+$i hour" +%s) | awk '{ if($5>=10) print $0}' |sort -u | head -1)
done

#calculate start-end for each pass
for x in $(printf -- '%s\n' "${var1[@]}" | grep : | awk '{print $1,$3$4}' | cut -d : -f 1,2 | sort -uk 2 | awk '{print $1}')
do
recstart=$(predict -t ~/wxsat/stations.txt -p "${bird}" $x | awk '{ if($5>=10) print $0}' | head -1 | awk '{print $1}')
recend=$(predict -t ~/wxsat/stations.txt -p "${bird}" $x | awk '{ if($5>=10) print $0}' | tail -1 | awk '{print $1}')
